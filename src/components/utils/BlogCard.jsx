  ...
  return (
    <>
      <Card>
        {operation && user._id == userId && (
          <Card.Header>
            ...
            <FontAwesomeIcon
              icon={faTrash}
              onClick={handleShow}
              style={{ fontSize: "1.3rem", cursor: "pointer", color: "red" }}
            />
          </Card.Header>
        )}
        ...
        <Card.Img
          variant="top"
          src="https://via.placeholder.com/100x50.png/"
        ></Card.Img>
        <Card.Body>
          <Card.Title>
            <OverlayTrigger
              key={blog._id}
              placement={`top`}
              overlay={<Tooltip id={blog._id}>{blog.title}</Tooltip>}
            >
              <Link className="link" key={blog._id} to={`/blog/${blog._id}`}>
